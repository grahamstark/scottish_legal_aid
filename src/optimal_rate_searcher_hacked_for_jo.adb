...
package body optimal_rate_searcher_hacked_for_jo is
   ...
   --
   function calc_income_group( bu : Model_Benefit_Unit; excess_income : Money; equivalise : boolean := false ) return Income_Counter is
      increment        : constant Money := 250.0;
      limit            : Money := increment;
      equiv_income     : Money := excess_income;
   begin
      if( equivalise ) then
         equiv_income := Equivalence_Scales.equivalise_income( bu, excess_income );
      end if;
   ...
   begin
      ...
      rates_loop: for r in Rate_Counter loop
         ...
         -- 
            -- case l is
               -- when 1 => limit := 10_725.0;rate := 1.0/3.0;
               -- when 2 => limit := 10_851.0;rate := 1.0;
               -- when 3 => limit := 11_841.0;rate := 1.0/3.0;
               -- when 4 => limit := 12_551.0; rate := 1.0; 
               -- when 5 => limit := 13_294.0;rate := 1.0/3.0;
               -- when 6 => limit := 15_481.0;rate := 1.0;
            -- end case;
            
            case l is
               when 1 => limit := 10_725.0;rate := 1.0/3.0;
               when 2 => limit := 10_725.0;rate := 1.0;
               when 3 => limit := 11_841.0;rate := 1.0/3.0;
               when 4 => limit := 11_841.0; rate := 1.0; 
               when 5 => limit := 13_294.0;rate := 1.0/3.0;
               when 6 => limit := 13_294.0;rate := 1.0;
            end case;                      
            ...
            benefit_units_loop: for buno in 1 .. mhh.num_benefit_units loop
               ...
               if(( base_output( buno ).la_State = disqualified ) and ( output( buno ).la_State /= disqualified )) then
                  ...
                  income_band := calc_income_group( mhh.benefit_units( buno ), excess_income, equivalise );
   ...
   function generate_searches( sys : Legal_Aid_Sys; control : Control_Record; equivalise : boolean ) return Cell_Array is
   ...
   begin
      ...
      for year in  settings.start_year .. settings.end_year loop
         ...
         for href in  startHH .. endHH loop
            ...
            generate_matrix_for_hh( cells, sys, mhh, settings, grossing_factor, ctl, base_output, equivalise );
        end loop; -- households
        hh_io.Close (hh_file);
      end loop; -- years
      new_line;
   ...
   procedure print_searches( file_name : String;  cells : Cell_Array ) is
      outfile : File_Type;
      limit   : Money := 9_781.0;
    begin
