...
package body optimal_rate_searcher is
   ...
   function calc_income_group( bu : Model_Benefit_Unit; excess_income : Money ) return Income_Counter is
      increment        : constant Money := 500.0;
      limit            : Money := 0.0;
      ...
      rates_loop: for r in Rate_Counter loop
         ...
         limits_loop: for l in Limit_Counter loop
            ...
            benefit_units_loop: for buno in 1 .. mhh.num_benefit_units loop
               ...
               for t in LA_Problem_Type loop
                     cells( r, l ).cost_by_problem(  t ) :=  
                     cells( r, l ).cost_by_problem( t ) + SReal( 
                        output( buno ).costs( civil, expected_net_costs, t ) - base_output( buno ).costs( civil, expected_net_costs, t ) ) * gross; 
                     cells( r, l ).cases_by_problem(  t ) :=  
                     cells( r, l ).cases_by_problem( t ) + SReal( 
                        output( buno ).costs( civil, expected_takeup, t ) - base_output( buno ).costs( civil, expected_takeup, t ) ) * gross; 
               end loop;
               --
               ...
               if(( base_output( buno ).la_State = disqualified ) and ( output( buno ).la_State /= disqualified )) then
                  ...
                  cells( r, l ).cost_by_income( income_band ) := 
                     cells( r, l ).cost_by_income( income_band ) + ( total_cost * gross );
    ...
    begin
      ...
      for r in Rate_Counter loop
         for l in Limit_Counter loop
            ...
            put( outfile, ", " );               
            for c in LA_Problem_Type loop
               put( outfile,  format_utils.format( cells( r, l ).cost_by_problem(c) ) );
               put( outfile, ", " );                              
            end loop;
            for c in LA_Problem_Type loop
               put( outfile,  format_utils.format( cells( r, l ).cases_by_problem(c) ) );
               put( outfile, ", " );                                          
            end loop;
            for i in Income_Counter   loop
               put( outfile,  format_utils.format( cells( r, l ).cost_by_income(i) ) );
               put( outfile, ", " );                                          
            end loop;
            for i in Income_Counter   loop
               put( outfile,  format_utils.format( cells( r, l ).cost_by_income(i) ) );
               if( i < Income_Counter'Last ) then
                  put( outfile, ", " );                                          
               end if;
            end loop;
            put( outfile,  " )" );
