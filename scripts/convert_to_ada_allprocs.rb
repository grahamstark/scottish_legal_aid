...
class NameAndType
        ...
        def initialize( _name, _type = 'modelint' )
                @name = _name;
                @type = _type;
        end
...
class PositionIndicator
        ...
        # add the position of some households record 
        # sernum frs serial number (a string)
        # record index of this record (for example, Adult=0 .. )
        # pos array of file positions (lines) of this record in the datafile and 
        #
        def add( sernum, record, pos )
                ## to integer so it sorts properly
                seri = sernum.to_i
                if( ! @positions.has_key?( seri ) )then
                        @positions[ seri ] = newRow();
                end
                @positions[ seri ][ record ] << pos;
        end;
...
def makeAdaRecord( recname, variables )
        ...
        record += INDENT*2 + "hh_sequence : modelint := MISS;\n";
        variables .size.times{
                |p|
                if( p > 0 ) then ## 0 is sernum, which we have already.
                        if( variables[p].type == 'modelint' ) then
                                misstype = 'MISS' 
                        else 
                                misstype = 'MISS_R'
                        end
...
class CSVParser
                ...
                @variables.size.times{
                        ...
                        if( p > 0 )then
                                ...
                                case( type ) ## reals that look like ints
                                        when 'real' then  reader = 'real_io';
                                        when 'modelint' then reader = 'int_io';
        ...
        def cleanupItem( item, type )
              if( ( not item.nil?) and ( item.length > 0) and ( item != 'A' ) and ( item != "-1" ) and ( item != '-1.0' )) then 
              ## not just blanks or missing value indicators?f (-1) 
                      if( type == 'real' )then ## reals that look like ints
                              if( item[0,1] == "." ) then
                      ...
                      item = (type == 'modelint') ? MISS : MISS_R;
              end
              return item
        end;
		...
		p = 0;
                CSV::Reader.parse( file, @sep ){
                        |elements|
			if( p > 0 ) then
	                        outfile.write( convertDataLine( elements ))
			end
			p = p+1
                }
                file.close();
                ...
                CSV::Reader.parse( file, @sep ){
                        ...
                        if( pos == 0 )then ## parse top line into an array of variable names
                                elements.each{
                                        ...
                                        @variables  << NameAndType.new( varname,'modelint' );
                                }
...
parseOneFile( ['0304','0405'],'accounts', "Account", 4, hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'adult', "Adult", 3, hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'admin', "Admin", 3, hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'assets', "Asset", 5, hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'benefits', "Benefit", 4, hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'benunit', "Benunit", 2, hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'care', "Care", 3, hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'child', "Child", 3, hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'endowmnt', "Endowment", 3, hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'extchild', 'ExtChild', 3, hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'govpay', 'GovPay', 4, hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'househol', 'Household', 1,  hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'insuranc', 'Insurance', 2,  hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'job', 'Job', 4,  hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'maint', 'Maint', 4,  hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'mortcont', 'MortCont', 2,  hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'mortgage', 'Mortgage', 2,  hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'oddjob', 'OddJob', 5,  hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'owner', 'Owner', 1,  hhrefs , posIndicator)
parseOneFile( ['0304','0405'],'penprov', 'PenProv', 3, hhrefs, posIndicator )   
parseOneFile( ['0304','0405'],'penamt', 'PenAmt', 3, hhrefs, posIndicator )   
parseOneFile( ['0304','0405'],'pension', 'Pension', 3, hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'rentcont', 'RentCont', 2, hhrefs, posIndicator )
parseOneFile( ['0304','0405'],'renter', 'Renter', 1, hhrefs, posIndicator )
parseOneFile( ['0304'],'vehicle', 'Vehicle', 1, hhrefs, posIndicator )
